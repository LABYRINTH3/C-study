...
// 기존에 배운 내용으로는 해당 연산자 구현이 힘들었지만 5.27일에 배운 내용을 바탕으로 코드를 수정했습니다.
ostream& operator>>(ostream &printdata, const Record &Record) {
    printdata << "이름: " << Record.name << "| 팔굽혀 펴기: " << Record.pushup << "| 50m 달리기: " << Record.run << "| 턱걸이: " << Record.pullup << "| 제자리 멀리 뛰기: " << Record.jump
        << "| 윗몸일으키기: " << Record.situp << endl;
    return printdata;
}
